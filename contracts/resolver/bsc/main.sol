...
contract FlashResolverBSC is Helper {
        ...
        )
    {
        ...
        for (uint256 i = 0; i < routesWithAvailability_.length; i++) {
            if (routesWithAvailability_[i] == 12) {
                address pair = getPancakeBestPair(_tokens, _amounts);
                uint256 pancakeFeeBPS = 25; // constant level fee for pancake swap
                uint256 instaFeeBps_ = flashloanAggregator.InstaFeeBPS();
                if (pancakeFeeBPS < instaFeeBps_) {
                    pancakeFeeBPS = instaFeeBps_;
                }
                if (feeBPS_ > pancakeFeeBPS) {
                    feeBPS_ = pancakeFeeBPS;
                    bRoutes_[0] = routesWithAvailability_[i];
                    _data[0] = abi.encode(pair);
                    j = 1;
                } else if (feeBPS_ == pancakeFeeBPS ) {
                    bRoutes_[j] = routesWithAvailability_[i];
                    _data[j] = abi.encode(pair);
                    j++;
                }
...
contract InstaFlashResolverBSC is FlashResolverBSC {
