...
import {
  ...
  EquilizerImplementation__factory,
  AaveImplementation,
  BalancerImplementation,
  MakerImplementation,
  UniswapImplementation,
  EquilizerImplementation
} from '../../typechain'
...
describe('FlashLoan', function () {
  let Aggregator,
    ...
    proxyE : EquilizerImplementation,
    ProxyA,
    ProxyB,
    ProxyM,
    ProxyU,
  ...
  const Dai = ethers.utils.parseUnits('5', 16)
  ...
  beforeEach(async function () {
    ...
    ProxyE = new EquilizerImplementation__factory(signer)
    proxyE = await ProxyE.deploy()
    await proxyE.deployed()
    console.log("Equilizer proxy deployed at: ", proxyE.address);
    ...
    const data = iface.encodeFunctionData('initialize', [
      ...
      ],master,proxyA.address,proxyB.address,proxyM.address,proxyU.address, proxyE.address
    ])
  ...
  describe('Single token', async function () {
    ...
    // })
    it('Should be able to take flashLoan of a single token from Equilizer', async function () {
  ...
  //   })
    ...
    //   )
    // })
    // it('Should be able to take flashLoan of multiple tokens together from Equilizer', async function () {
    //   await receiver.flashBorrow(
    //     [DAI, USDT, WETH],
    //     [Dai, Usdt, Weth],
    //     9,
